---
title: "DevOps Project-38: End-to-End Deployment of a FullStack Blogging Application with AWS EKS, Terraform, Jenkins, SonarQube, Nexus, Trivy & Prometheus/Grafana"
summary: This project demonstrates the complete CI/CD automation and deployment of a FullStack Blogging Application built with Spring Boot. The application is deployed on Amazon Elastic Kubernetes Service (EKS) using Terraform for Infrastructure as Code (IaC). The pipeline is managed through Jenkins and integrates with SonarQube for code quality checks, Nexus for artifact management, and Trivy for container image security scanning. The project also incorporates Prometheus and Grafana for monitoring and observability, delivering a production-ready DevOps workflow aligned with enterprise best practices.
objective: Automate the provisioning, build, testing, security scanning, artifact management, and deployment of a FullStack Spring Boot application on AWS EKS. By the end of this project, you will have implemented a secure, observable, and scalable DevOps pipeline, showcasing practical experience with Kubernetes, IaC, CI/CD automation, and cloud-native monitoring.
tone:
  - technical
  - hands-on
  - cloud-native
...
resource "aws_vpc" "hemanshu-dev_vpc" {
  cidr_block = "10.0.0.0/16"
  tags = {
    Name = "hemanshu-dev-vpc"
  }
}
resource "aws_subnet" "hemanshu-dev_subnet" {
  count = 2
  vpc_id                  = aws_vpc.hemanshu-dev_vpc.id
  cidr_block              = cidrsubnet(aws_vpc.hemanshu-dev_vpc.cidr_block, 8, count.index)
  availability_zone       = element(["ap-southeast-1a", "ap-southeast-1b"], count.index)
  map_public_ip_on_launch = true
  tags = {
    Name = "hemanshu-dev-subnet-${count.index}"
  }
}
resource "aws_internet_gateway" "hemanshu-dev_igw" {
  vpc_id = aws_vpc.hemanshu-dev_vpc.id
  tags = {
    Name = "hemanshu-dev-igw"
  }
}
resource "aws_route_table" "hemanshu-dev_route_table" {
  vpc_id = aws_vpc.hemanshu-dev_vpc.id
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.hemanshu-dev_igw.id
  }
  tags = {
    Name = "hemanshu-dev-route-table"
  }
}
resource "aws_route_table_association" "a" {
  count          = 2
  subnet_id      = aws_subnet.hemanshu-dev_subnet[count.index].id
  route_table_id = aws_route_table.hemanshu-dev_route_table.id
}
resource "aws_security_group" "hemanshu-dev_cluster_sg" {
  vpc_id = aws_vpc.hemanshu-dev_vpc.id
  egress {
    from_port   = 0
    to_port     = 0
  ...
  tags = {
    Name = "hemanshu-dev-cluster-sg"
  }
}
resource "aws_security_group" "hemanshu-dev_node_sg" {
  vpc_id = aws_vpc.hemanshu-dev_vpc.id
  ingress {
    from_port   = 0
    to_port     = 0
  ...
  tags = {
    Name = "hemanshu-dev-node-sg"
  }
}
resource "aws_eks_cluster" "hemanshu-dev" {
  name     = "hemanshu-dev-cluster"
  role_arn = aws_iam_role.hemanshu-dev_cluster_role.arn
  vpc_config {
    subnet_ids         = aws_subnet.hemanshu-dev_subnet[*].id
    security_group_ids = [aws_security_group.hemanshu-dev_cluster_sg.id]
  }
}
resource "aws_eks_node_group" "hemanshu-dev" {
  cluster_name    = aws_eks_cluster.hemanshu-dev.name
  node_group_name = "hemanshu-dev-node-group"
  node_role_arn   = aws_iam_role.hemanshu-dev_node_group_role.arn
  subnet_ids      = aws_subnet.hemanshu-dev_subnet[*].id
  scaling_config {
    desired_size = 3
    max_size     = 3
  ...
  remote_access {
    ...
    source_security_group_ids = [aws_security_group.hemanshu-dev_node_sg.id]
  }
}
resource "aws_iam_role" "hemanshu-dev_cluster_role" {
  name = "hemanshu-dev-cluster-role"
...
resource "aws_iam_role_policy_attachment" "hemanshu-dev_cluster_role_policy" {
  role       = aws_iam_role.hemanshu-dev_cluster_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSClusterPolicy"
}
resource "aws_iam_role" "hemanshu-dev_node_group_role" {
  name = "hemanshu-dev-node-group-role"
  assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_role_policy_attachment" "hemanshu-dev_node_group_role_policy" {
  role       = aws_iam_role.hemanshu-dev_node_group_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy"
}
resource "aws_iam_role_policy_attachment" "hemanshu-dev_node_group_cni_policy" {
  role       = aws_iam_role.hemanshu-dev_node_group_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"
}
resource "aws_iam_role_policy_attachment" "hemanshu-dev_node_group_registry_policy" {
  role       = aws_iam_role.hemanshu-dev_node_group_role.name
...
```plaintext
output "cluster_id" {
  value = aws_eks_cluster.hemanshu-dev.id
}
output "node_group_id" {
  value = aws_eks_node_group.hemanshu-dev.id
}
output "vpc_id" {
  value = aws_vpc.hemanshu-dev_vpc.id
}
output "subnet_ids" {
  value = aws_subnet.hemanshu-dev_subnet[*].id
}
```
...
```plaintext
aws eks --region ap-southeast-1 update-kubeconfig --name hemanshu-dev-cluster
```
...
kubectl create secret docker-registry regcred \
  ...
  --docker-username=hemanshu-dev \
  --docker-password=<your_dockerhub_password> \
  --docker-email=hemanshu-dev@gmail.com \
  --namespace=webapps
```
...
```plaintext
git branch: 'main', credentialsId: 'git-cred', url: 'https://github.com/hemanshu-dev/Boardgame.git'
```
...
This project is crafted by [**Hemanshu**](https://github.com/NotHemanshu) 💡.  
...
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/Hemanshu-vardhan-reddy) [![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/NotHemanshu) [![Telegram](https://img.shields.io/badge/Telegram-26A5E4?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/prodevopsguy) [![Dev.to](https://img.shields.io/badge/Dev.to-0A0A0A?style=for-the-badge&logo=dev.to&logoColor=white)](https://dev.to/notHemanshu) [![Hashnode](https://img.shields.io/badge/Hashnode-2962FF?style=for-the-badge&logo=hashnode&logoColor=white)](https://hashnode.com/@prodevopsguy)
